...
class CL61Classifier:
    ...
    def _load_config(self, config_path):
        """
        Loads a json config file from given relative or absolute config file path
        Args:
        ...
        for variable_name, transform in zip(variable_names, transforms):
            ...
            if transform == 'log':
                if np.any(feature_array < 0):
                    ...
                    print(
                        f"Warning: Found negative or zero values in '{variable_name}'. Log transformation may result in NaN.")
    ...
    def Kmeans_clustering(self, cluster_N=8,
        ...
        feature_matrix, nan_rows, cleaned_feature_matrix = self.dataset_to_sample_feature(
        ...
        if plot_result:
            ...
            fig, axes[0] = visualize_classification_featurespace_2D(log10_beta_att_flatten=cleaned_feature_matrix[:, 0],
                                                                    linear_depol_ratio_flatten=cleaned_feature_matrix[:,
                                                                                               1],
        ...
        self.dataset[new_var].attrs[
            'description'] = f'{cluster_N} K-mean clusters based on the following variables as features: {variable_as_features}'
    ...
    def dbscan_clustering(self, variable_as_features=['beta_att', 'linear_depol_ratio', 'range'],
        ...
        self.dataset[new_var].attrs[
            'description'] = f'DBSCAN clustering with eps={dbscan_eps}, min_samples={dbscan_min_samples}'
    ...
    def classify_clusters(self,
        ...
        Args:
            cluster_variable: Variable containing the cluster number of each datapoint. Defaults to 'kmean_clusters'.
            beta_attenuation_varname: Variable name for beta attenuation. Default is 'beta_att_clean'.
            linear_depol_ratio_varname: Variable name for linear depolarization ratio.
                                        Default is 'linear_depol_ratio_clean'.
            verbose (bool, optional): if True, prints information on process
        ...
        for cluster_id in np.unique(self.dataset[cluster_variable]):
            ...
            mean_beta = xr.where(mask_cluster, self.dataset[beta_attenuation_varname], np.nan).mean(
                skipna=True)  # NA should not be present but making sure
            mean_depol = xr.where(mask_cluster, self.dataset[linear_depol_ratio_varname], np.nan).mean(
                skipna=True)  # same
    ...
    def classify_elementwise(self,
        ...
        """
        Classifies element-wise based on thresholds focused on the variables in question. Saves the results
        into new variable with given name.
        Args:
            beta_attenuation_varname (str): variable name of the attenuated backscatter array.
                                            Default to 'beta_att_clean'.
            linear_depol_ratio_varname (str): Variable name of the linear depolarization ratio array.
                                            Default to 'linear_depol_ratio_clean'.
            new_var_name (str): Variable name to store the resulting array. Default to 'classified_elements'.
        """
...
def threshold_classify_element(class_config, beta_attenuation, linear_depolarization_ratio):
    """
    Finds based on a pair of measures (beta attenuation, linear depolarization ratio) the class related to it
    Args:
        class_config (dict): Dictionary containing all classes wich contain their value ranges stored in variables
         ('beta_attenuation_range', 'linear_depolarization_ratio')
    returns:
        the class id related the pair of measurement.
    """
